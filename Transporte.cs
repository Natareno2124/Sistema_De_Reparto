...
namespace WinFormsApp1
{
    public class Transporte : BaseForm
    {
        ...
        public Transporte()
        {
            ...
            cmbRutas.Items.AddRange(new string[] { "Seleccionar Sucursal", "Sur", "Norte" });
            ...
            cmbOrdenar.Items.AddRange(new string[] { "Ordenar por: ID", "Ordenar por: Placa", });
            ...
            ContentPanel.Controls.Add(panelFiltros);
            ContentPanel.Controls.Add(dgvVehiculos);

            dgvVehiculos.ColumnHeadersDefaultCellStyle.Padding = new Padding(0, 5, 0, 5);
            dgvVehiculos.ColumnHeadersDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#2C546D"); 
            dgvVehiculos.ColumnHeadersDefaultCellStyle.ForeColor = Color.White; // Para el texto
            dgvVehiculos.ColumnHeadersHeight = 40;
            dgvVehiculos.EnableHeadersVisualStyles = false;
            dgvVehiculos.GridColor = Color.White;
            //solo lineas verticales 
            dgvVehiculos.CellBorderStyle = DataGridViewCellBorderStyle.SingleVertical;
            // color de las líneas
            dgvVehiculos.GridColor = ColorTranslator.FromHtml("#CCCCCC");

            dgvVehiculos.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            dgvVehiculos.MultiSelect = false; // Para que solo se seleccione una fila
        ...
        {
            dgvVehiculos.DataSource = vehiculosDAO.ObtenerVehiculos();
        }
        ...
        private void BtnAgregar_Click(object sender, EventArgs e)
        {
            ...
            {
                vehiculosDAO.InsertarVehiculo(
                    form.PlacaValue,
                    form.MarcaValue,
                    form.ModeloValue,
                    form.ColorValue,
                    form.CapacidadValue,
                    form.IdSucursalValue,
                    form.TipoValue,
                    form.EstadoValue
                );
                MostrarConductores();
        ...
        private void BtnEditar_Click(object sender, EventArgs e)
        {
            ...
            FormVehiculo form = new FormVehiculo
            {
                PlacaValue = Convert.ToInt32(row.Cells["iD_Placa"].Value),
                MarcaValue = row.Cells["marca"].Value.ToString(),
                ModeloValue = row.Cells["modelo"].Value.ToString(),
                ColorValue = row.Cells["color"].Value.ToString(),
                CapacidadValue = row.Cells["capacidad"].Value.ToString(),
                IdSucursalValue = Convert.ToInt32(row.Cells["id_Sucursal"].Value),
                TipoValue = Convert.ToInt32(row.Cells["id_tipo"].Value),
                EstadoValue = Convert.ToInt32(row.Cells["id_estado"].Value)
            };

            if (form.ShowDialog() == DialogResult.OK)
            {
                vehiculosDAO.ModificarVehiculo(
                    form.PlacaValue,
                    form.MarcaValue,
                    form.ModeloValue,
                    form.ColorValue,
        ...
        private void BtnEliminar_Click(object sender, EventArgs e)
        {
            if (dgvVehiculos.SelectedRows.Count > 0)
            {
                int placa = Convert.ToInt32(dgvVehiculos.SelectedRows[0].Cells["iD_Placa"].Value);
                if (MessageBox.Show("¿Eliminar este vehículo?", "Confirmar", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    vehiculosDAO.EliminarVehiculo(placa);
                    MostrarConductores();
                }
            }
            else
